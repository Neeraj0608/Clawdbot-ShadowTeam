...
export default function LoginPage() {
  ...
  const handleLogin = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (data?.needsVerification || data?.needsOtp) {
        setVerificationEmail(data?.email || "");
        setNeedsOtp(true);
        const msg = data?.needsOtp
          ? "Enter the OTP sent to your email to login."
          : "Verify your email with the OTP we just sent.";
        setStatus(msg);
        pushToast(msg, "info");
        return;
      }
  ...
  const verifyOtp = async (otpCode: string) => {
    ...
    try {
      ...
      // If we got session data, log the user in directly
