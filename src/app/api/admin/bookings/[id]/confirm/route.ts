...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Send push notification via WebSocket server
    try {
      await fetch('http://localhost:8001/notify', {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          message: JSON.stringify({
            ...
            message: `Good news! Your booking for ${booking.lab} on ${booking.date.toISOString().split('T')[0]} has been approved.`,
            bookingId: booking.id
          })
        })
      });
    } catch (wsErr) {
      console.error('Failed to send push notification:', wsErr);
    }
