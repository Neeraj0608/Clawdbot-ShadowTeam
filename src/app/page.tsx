...
export default async function HomePage({
  ...
  const [heroSlidesRaw, newsRaw, events, grants, announcements, openHackathons, openProblems, scrapedOpportunities] =
    await Promise.all([
      ...
      prisma.scrapedOpportunity.findMany({
        where: {
          isActive: true,
  ...
  return (
    <main className="max-w-[1560px] mx-auto grid grid-cols-12 gap-0 min-h-screen pt-[100px] sm:pt-[108px] md:pt-[120px]">
      ...
      <div className="col-span-12 md:col-span-9 lg:col-span-9 p-3 sm:p-4 md:p-8 lg:p-12 bg-[#fffefc] md:bg-white md:border-x border-[#d8dae6] md:shadow-[0_0_0_1px_rgba(0,33,85,0.03)]">
        ...
        <section id="live-opportunities" className="mt-12 mb-14">
          <div className="border-l-4 border-[#8c4f00] pl-4 md:pl-6 mb-6">
            ...
            <p className="text-xs uppercase tracking-widest text-[#8c4f00] mt-1">
              Latest Internships & Hackathons (Scraped Live)
            </p>
          </div>

          {/* Filters */}
          <form method="GET" className="flex flex-wrap gap-3 mb-8 items-end bg-[#f5f4f0] p-4 border border-[#c4c6d3]">
            <div className="flex-1 min-w-[200px]">
              <label className="block text-[9px] font-bold text-[#747782] uppercase tracking-widest mb-1">Search</label>
              <input name="q" defaultValue={query} placeholder="Keyword, company, skill…" className="w-full border border-[#c4c6d3] bg-white px-3 py-2 text-xs text-[#002155] focus:outline-none" />
            </div>
            <div>
              <label className="block text-[9px] font-bold text-[#747782] uppercase tracking-widest mb-1">Type</label>
              <select name="type" defaultValue={typeFilter ?? ''} className="border border-[#c4c6d3] bg-white px-3 py-2 text-xs text-[#002155]">
                <option value="">All Types</option>
                {types.map(t => <option key={t} value={t}>{t.charAt(0) + t.slice(1).toLowerCase()}</option>)}
              </select>
            </div>
            <button type="submit" className="px-5 py-2 bg-[#002155] text-white text-[10px] font-bold uppercase tracking-widest hover:bg-[#003080]">Filter</button>
            {(typeFilter || query) && (
              <Link href="/" className="px-5 py-2 border border-[#c4c6d3] text-[#747782] text-[10px] font-bold uppercase tracking-widest">Clear</Link>
            )}
          </form>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {scrapedOpportunities.length === 0 ? (
               <p className="col-span-full text-center py-10 border border-dashed border-[#c4c6d3] text-sm text-[#747782]">No opportunities found matching your filters.</p>
            ) : (
              scrapedOpportunities.map((opp: any) => {
                const typeColor = TYPE_COLORS[opp.type] ?? { bg: 'bg-[#eef0f5]', text: 'text-[#002155]' };
                return (
                  <article key={opp.id} className="border border-[#c4c6d3] bg-white p-4 flex flex-col gap-2 hover:shadow-md transition-shadow">
                    <div className="flex items-center gap-2">
                       <span className={`text-[8px] font-bold uppercase tracking-widest px-2 py-0.5 ${typeColor.bg} ${typeColor.text}`}>{opp.type}</span>
                       <span className="text-[8px] font-bold uppercase tracking-widest text-[#747782] border border-[#c4c6d3] px-2 py-0.5">{SOURCE_LABEL[opp.source]}</span>
                    </div>
                    <h3 className="font-semibold text-[#002155] text-xs leading-snug line-clamp-2">{opp.title}</h3>
                    <p className="text-[10px] text-[#747782]">{opp.company} • {opp.location}</p>
                    <p className="text-[11px] text-[#434651] line-clamp-2 mt-1">{opp.description}</p>
                    <div className="mt-auto pt-3 border-t border-[#f0efe9] flex items-center justify-between">
                      <span className="text-[9px] text-[#747782]">{opp.deadline ? `Due ${formatDate(opp.deadline)}` : 'No deadline'}</span>
                      <a href={opp.registrationLink} target="_blank" rel="noreferrer" className="text-[9px] font-bold text-[#8c4f00] uppercase underline">Apply →</a>
                    </div>
                  </article>
                )
              })
          ...
          <div className="mt-6 text-center">
            <Link href="/opportunities" className="inline-block px-8 py-3 bg-[#002155] text-white text-xs font-bold uppercase tracking-widest hover:bg-[#003080]">
              View All 100+ Opportunities
            </Link>
          </div>
        </section>
