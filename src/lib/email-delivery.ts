...
const getTransporter = () => {
  ...
  // If SMTP_PASS is provided, use standard username/password (App Password)
...
export const sendEmail = async ({ to, subject, html, attachments }: SendEmailInput) => {
  try {
    // DEV MODE BYPASS: If no real credentials are provided, just mock the email sending
    const isMock = !process.env.SMTP_USER || process.env.SMTP_USER.includes('your-email') || (!process.env.SMTP_PASS && getMissingOAuthEnvVars().length > 0);
    if (isMock) {
      console.log('\n=========================================================');
      console.log(`[DEV EMAIL MOCK] To: ${to}`);
      console.log(`[DEV EMAIL MOCK] Subject: ${subject}`);
      console.log(`[DEV EMAIL MOCK] Status: Bypassed real SMTP dispatch`);
      console.log('=========================================================\n');
