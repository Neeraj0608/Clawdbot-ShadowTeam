...
export const sendOTPEmail = async (email: string, otp: string) => {
  console.log(`\n======================================`);
  console.log(`[DEV] REGISTRATION OTP For: ${email}`);
...
export const sendLoginOTPEmail = async (email: string, otp: string) => {
  console.log(`\n======================================`);
  console.log(`[DEV] LOGIN OTP For: ${email}`);
  console.log(`[DEV] OTP CODE: ${otp}`);
  console.log(`======================================\n`);
  const body = `
    <h2 style="color:#002155;margin:0 0 8px;">Login Verification</h2>
    <p style="color:#434651;font-size:14px;">Use the following One-Time Password to complete your login:</p>
    <div style="background:#f5f4f0;border-left:4px solid #F7941D;padding:16px 24px;margin:16px 0;text-align:center;">
      <span style="font-size:32px;font-weight:bold;letter-spacing:8px;color:#002155;">${otp}</span>
    </div>
    <p style="color:#747782;font-size:12px;">This code is valid for <strong>10 minutes</strong>. Do not share it with anyone.</p>`;
  await send(email, `Your TCET CoE Login OTP: ${otp}`, body, {
    mode: 'immediate',
    category: 'AUTH_LOGIN_OTP',
  });
};
